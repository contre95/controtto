...
func Run(c *config.Service, m *managing.Service, q *querying.Service, port string) {
	...
	app.Get("/pairs/:id/", pairSection(c.ConfigManager))
	...
	app.Get("/settings", settingsSection(c.ConfigManager))
	app.Get("/settings/anyMarket", marketsSetAPI(c.ConfigManager))
	// app.Get("/markets", marketsSection(m.MarketManager))
	app.Get("/ui/pairs/:id/market", getMarketAssets(c.ConfigManager, q.TradingPairQuerier))
	app.Get("/ui/pairs/:id/newMarketTrade/form", newMarketTradingForm(c.ConfigManager, q.TradingPairQuerier))
	// DELETE
	...
	app.Post("/settings/edit", saveSettingsForm(c.ConfigManager))
