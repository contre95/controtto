...
func Run(c *config.Service, m *managing.Service, q *querying.Service, port string) {
	...
	app.Get("/healthcheck/price", checkPrice(q.PriceProviderManager))
	app.Get("/calc/price", calculatePrice(q.PriceProviderManager))
	app.Get("/dashboard", dashboardSection(q.TradingPairQuerier))
	app.Get("/ui/pairs/:id/cards", pairCards(q.TradingPairQuerier, q.PriceProviderManager))
	...
	app.Get("/ui/pairs/:id/newTrade/form", newTradeForm(q.TradingPairQuerier, q.PriceProviderManager))
	app.Get("/settings", settingsSection(q.PriceProviderManager, m.MarketManager, c.ConfigManager))
	...
	app.Post("/settings/edit", saveSettingsForm(q.PriceProviderManager, m.MarketManager, c.ConfigManager))
